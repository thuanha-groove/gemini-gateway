...
def _has_media_parts(contents: List[Dict[str, Any]]) -> bool:
    """Check if the message contains image, audio, or video parts (inline_data)"""
...
def _build_tools(
    ...
    # Merge tools from the request into the tools dictionary
    if request.tools:
            ...
            for fc in function_declarations:
                if fc.get("name") not in names:
                    if fc.get("name")=="googleSearch":
                        # When cherry's built-in search is enabled, add the googleSearch tool
                        tool["googleSearch"] = {}
                    else:
                        # Add other functions to functionDeclarations
...
class OpenAIChatService:
    ...
    def _extract_text_from_openai_chunk(self, chunk: Dict[str, Any]) -> str:
        """Extract text content from an OpenAI response chunk"""
    ...
    async def _fake_stream_logic_impl(
        ...
        """Core logic for handling fake stream"""
        logger.info(
        ...
        async def send_empty_data_locally() -> AsyncGenerator[str, None]:
            """Periodically send empty data to maintain the connection"""
    ...
    async def _real_stream_logic_impl(
        ...
        """Core logic for handling real stream"""
    ...
    async def _handle_stream_completion(
        ...
        """Handle streaming chat completion, with retry logic and fake stream support"""
        retries = 0
